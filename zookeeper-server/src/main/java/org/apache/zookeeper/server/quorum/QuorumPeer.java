 ...
 *   epoch持久化相关
 *
 * QuorumPeer中quorum代表的意思就是每个zookeeper集群启动的时候集群中zookeeper服务数量就已经确定了，在每个zookeeper的配置文件中配置集群中的所有机器
 *
...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public static class QuorumServer {
        ...
        public QuorumServer(long id, InetSocketAddress addr,
            ...
            this.addr = addr;//选举完之后，leader和learner的通信端口
            this.electionAddr = electionAddr;//选举之前，各server参与选举的port
            this.type = type;//server类型，为PARTICIPANT 或者 OBSERVER;
            this.clientAddr = clientAddr;// 客户端端口
    ...
    public enum ServerState {
        ...
        //LOOKING：初始状态，当服务器处于该状态时，它会认为当前集群中没有Leader，因此需要进入Leader选举状态。
        //FOLLOWING：跟随leader的角色，参与投票，表明当前服务器角色是Follower
        //LEADING：集群的leader
        //OBSERVING：不参与投票，只是同步状态，表明当前服务器角色是Observer。
    }
     ...
     * 在版本3.4.0中，此类已被弃用，因为它与其中一个基于udp的领导者选举版本一起使用，我们也在弃用。
     *
     * This class simply responds to requests for the current leader of this
     * node.该类仅响应对此节点的当前领导者的请求。
     * <p>
     * The request contains just an xid generated by the requestor.请求仅包含请求者生成的xid。
     * <p>
    ...
    synchronized public void startLeaderElection() {
        try {
            if (getPeerState() == ServerState.LOOKING) {
                // 如果服务的状态是LOOKING，创建Vote对象，此时是服务器第一次启动选举
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            // 一般使用这个，请认准此算法
            // 选举算法的创建之前先创建QuorumCnxManager,QuorumCnxManager利用tcp协议来进行leader选举，
            // 每一对server之间都会保持一个tcp链接，每个zookeeper会打开一个tcp/ipj监听集群中的其他server。
            ...
            if(listener != null){
                // Listener线程任务， 绑定地址开启ServerSocket服务，用来侦听其他server连接，
                // 进行集体间选举，投票， 数据同步。Server链接数有限，是基于bio的长连接。
                listener.start();
                // FastLeaderElection：创建选举对象
